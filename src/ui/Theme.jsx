...
function Theme() {
  ...
  return (
    <div className="relative min-w-lg:z-50">
      <Listbox value={theme} onChange={setTheme}>
        ...
        */}
        <div className="relative w-auto sm:w-36">
          ...
          <ListboxButton className="relative flex w-full cursor-pointer items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white p-2 text-left text-slate-900 shadow-sm transition-all hover:border-blue-400 focus:outline-none sm:justify-between sm:px-3.5 sm:py-2 dark:border-slate-800 dark:bg-black dark:text-white">
            {({ open }) => (
              <>
                ...
                <i
                  className={`fa-solid ${open ? "fa-chevron-up" : "fa-chevron-down"} transition-all duration-600 hidden text-[10px] text-slate-400 sm:inline`}
                ></i>
              </>
            )}
          </ListboxButton>
          ...
          */}
          <ListboxOptions className="absolute top-full right-0 z-50 mt-2 min-w-[130px] overflow-hidden rounded-2xl border border-slate-200 bg-white/95 p-1.5 shadow-2xl backdrop-blur-md focus:outline-none sm:w-full dark:border-slate-800 dark:bg-[#181818]/95 transition-all duration-600">
